...
vec4 grid(vec3 fragPos3D, float scale) { 
    ...
    vec4 colour = vec4(0, 0, 0, 1.0 - min(line, 1.0));

    colour.z += (float(((fragPos3D.x > -1.0 * minimumX) && (fragPos3D.x < 1.0 * minimumX))) * 0.8);
    colour.x += (float(((fragPos3D.z > -1.0 * minimumZ) && (fragPos3D.z < 1.0 * minimumZ))) * 0.8);
