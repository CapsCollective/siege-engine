...
public:
    ...
    struct Data
    {
        Utils::HeapArray<VkVertexInputBindingDescription> bindings;
        Utils::HeapArray<VkVertexInputAttributeDescription> attributes;
    };
...
public:
     ...
     * @param restartEnabled Allows a special vertex value to restart the assembly. Cannot be true
     *with list topologies. Defaults to false.
     ...
     * @param rasterizerDiscardEnable controls if primitives are discarded immediately before
     *rasterisation stage.
     ...
     * @param depthBiasConstantFactor specifies a constant depth for all fragments. Defaults to
     *0.0f.
     * @param depthBiasClamp the maximum depth bias value for a fragment. Defaults. 0.0f.
     * @param depthBiasSlopeFactor scalar factor applied for a fragment's depth bias. Defaults to
     *0.0f.
     ...
     * work for our pipelines. For more info, please check:
     ...
     * @param minSampleShading if sample shading is enabled, specify the minimum fraction fraction
     *to use.
     * @param pSampleMask a pointer to an array of sample masks to be used for multi-sampling.
     * @param alphaToCoverageEnable specifies if a temporary coverage value is generated for the
     *alpha component.
     * @param alphaToOneEnable specifies if an alpha component is replaced in accordance to
     *multi-sampling rules.
     ...
     * @param srcAlphaBlendFactor specifies the blend factor to be used for alpha values on the
     *source.
