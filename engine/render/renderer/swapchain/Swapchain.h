 ...
 *
 * The swapchain is therefore responsible for buffering these images by writing to a new
 * commandbuffer each frame. The new commandbuffer is then swapped out each frame depending on the
 * method used. This results in smooth image transitions and safer memory usage, as frames no longer
 * compete to render the same data.
 *
...
private:
    ...
    /**
     * @brief Create a Depth Resources object. Depth resources represent how object depth is
     * calculated. These resources are written to their own resource views.
     */
    void CreateDepthResources();

    /**
     * @brief Creates the swapchain's frame buffers. A frame buffer represents all the data needed
     * to be submitted to the GPU in a frame.
     */
    void CreateFrameBuffers();

    /**
     * @brief Initialises all thread synchronisation objects. Vulkan is multi-threaded and submits
     * images asynchronously. This can be dangerous when submitting images to the GPU since Vulkan
     * does not guarantee safe execution. Therefore we must handle our own thread synchronisation.
     */
