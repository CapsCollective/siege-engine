...
inline constexpr unsigned char BYTE_SIZE_IN_BITS = 8;
// The default object size for each segment (1 byte)
inline constexpr unsigned char BYTE_MASK_SIZE = sizeof(unsigned char);
// Each position represents a byte where all bits before the index are 1.
// i.e: index 2 = 0000 0111
inline constexpr unsigned char MAX_BIT_VALUES[BYTE_SIZE_IN_BITS] = {1, 3, 7, 15, 31, 63, 127, 255};
 ...
 */
inline constexpr const unsigned long CalculateBitSetSize(const unsigned long& arraySize)
{
 ...
 * @return
 */
inline constexpr unsigned char BitAtIndex(const unsigned long& index)
{
 ...
 * @return
 */
inline constexpr unsigned long Byte(const unsigned long& index)
{
 ...
 */
inline constexpr void SetBitToZero(unsigned char& byte, const unsigned long& bit)
{
    byte &= ~BitAtIndex(bit);
}
 ...
 * @return
 */
constexpr inline bool IsSet(const unsigned char* bitfield, const unsigned long& bit)
{
 ...
 */
inline constexpr bool IsInBounds(const unsigned long& index, const unsigned long& size)
{
    return index < size;
}
...
public:
    ...
    void UnsetBit(const unsigned long& bit)
    {
        leftMostBit = ::Siege::BitUtils::UnsetBit(bitfield, bit, leftMostBit, S);
    }
    ...
    {
        ::Siege::BitUtils::Clear(bitfield, S);
        leftMostBit = 0;
    }
