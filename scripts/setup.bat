...
:SetupFreeType
    ...
    cmake -G "%GENERATOR%" -DFT_DISABLE_BROTLI=TRUE -DFT_DISABLE_BZIP2=TRUE -DFT_DISABLE_HARFBUZZ -DCMAKE_INSTALL_PREFIX=%BUILD_DIR% -DZLIB_LIBRARY=%ZLIB_BUILD_DIR%\lib\libz.a -DZLIB_INCLUDE_DIR=%ZLIB_BUILD_DIR%\include -DPNG_LIBRARY=%LIBPNG_BUILD_DIR%\libpng.a -DPNG_PNG_INCLUDE_DIR=%LIBPNG_BUILD_DIR% -B%BUILD_DIR% -S%VENDOR_DIR%\freetype -DCMAKE_SH="CMAKE_SH-NOTFOUND"
